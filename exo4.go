...
func getUserLetter() string {
	...
	for true {
		...
		if len(guess) != 1 {
			f.Println("Please Enter Only One Letter")
		} else if !IsLetter(guess) {
			f.Println("Please Enter a Letter")
		} else if strings.Contains(guessedLetters, guess) {
			f.Println("Please Enter a Letter you Haven't Guessed")
		} else {
			return guess
		}
	}
	...
	for true {
		...
		if strings.Contains(randWord, guess) {
			...
			if sliceHasEmptys(correctLetters) {
				f.Println("More Letters to Guess")
			} else {
				f.Println("Yes the Secret Word is", randWord)
				break
			}
			...
			if len(wrongGuesses) >= 6 {
				f.Println("Sorry Your Dead! The word is", randWord)
				break
			}
		}
	}
